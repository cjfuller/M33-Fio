...
if len(sys.argv) == 2 and (sys.argv[1] == "-h" or sys.argv[1] == "--help"):
    ...
    # Display usage
    print(
        "Webcam Server: Hosts a webcam's stream and snapshot at /stream.mjpg and /snapshot.jpg\nUsage: python webcam_server.py camera httpPort framesPerSecond width height"
    )
...
# Otherwise
    ...
    print(
        (
            ...
            + str(cameraHeight)
            + " running at "
            + str(int(1.0 / cameraFrameDelay))
            + " frames/second"
        )
    )
    print(
        (
            "Hosting webcam still image at http://"
            + ipAddress
            + ":"
            + str(httpPort)
            + "/snapshot.jpg"
    ...
    print(
        (
            "Hosting webcam video stream at http://"
            + ipAddress
            + ":"
            + str(httpPort)
            + "/stream.mjpg"
        ...
        while True:
            ...
            try:
                ...
                rawImage = Image.frombytes(
                    ...
                    pygame.image.tostring(cameraImage, "RGB", False),
                )
    ...
    # Otherwise check if QTKit is usable
        ...
        class Camera(NSObject):
            ...
            def captureOutput_didOutputVideoFrame_withSampleBuffer_fromConnection_(
                ...
                bitmapRepresentation = NSBitmapImageRep.alloc().initWithCIImage_(
                    rawImage
                )
                bitmapData = bitmapRepresentation.representationUsingType_properties_(
                    NSJPEGFileType, {NSImageCompressionFactor: 1.0}
                )
